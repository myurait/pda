...
18. 拒否上限の計上規則そのものを固定する。理由コードから計数先への写像、免除経路がクラス予算で有界であること（免除される理由コードごとに固定する）、逸脱試行が上限を枯渇させること、および **invocation から理由コードへの分類が両方向で正しいこと**（書込形・境界外読み取りは計上側、境界内の純粋な読み取りは免除側）。写像のみを固定すると、分類の誤りが検出されずに残る。分類の両方向は**許可された読み取り subcommand と認識のみの読み取り subcommand の双方**について固定し、パスが結合値やフラグに詰めた形（束ねたフラグの末尾に置いた値取り形を含む）で運ばれる場合も含める。片側の集合だけを固定すると、同じ粒度の誤りが他方の集合で残る。免除側の固定は**値にパス区切りを含む正当な形**（検索パターン・書式文字列・表示接頭辞・行範囲の正規表現）を含めて行う。値がパスを含まない形だけを固定すると、回帰が住む区画を通らない。宣言によって分類が決まる箇所は、**宣言表が認識集合上で全域であること**自体を固定する（宣言の欠落は、欠落した族に対して分類の誤りを原理的に検出不能にする）。
...
lock されていない artifact-change ターンでは、既知の読み取り系ツール（read/search/list 相当）と作業記録系カテゴリの注記系（D-S3-7、「第一層」参照。run 終端シグナル系は locked 段のみ）だけを許可し、監査記録のみを行う。構造化書込・変異系ツール、`terminal` を含む実行系、未知ツールは既定拒否とし、理由コードを「lock 未了」とする。拒否の計上は「第一層」の計上規則に従う（write 境界・実行境界への逸脱試行のみを拒否上限へ計上する）。読み取り専用 git は lock 前段では許可しない。照合基準となるロック済み root がまだ存在せず、workdir 束縛を検査できないためである。
