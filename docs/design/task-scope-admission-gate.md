...
**実機で新たに判明した事項（未処置、オーナー判断待ち）**: 読み取り専用ツール（スキル定義閲覧・ツール schema 閲覧）が「第一層: write 境界」節の読み取り系ツール許可集合に含まれないため拒否される事例を確認した。非計上・座礁なしだが純粋読み取りへの運用上の摩擦になる。カタログへの追加は許可集合を広げる変更に当たるため本ラウンドでは実装せず、有効化ゲートのオーナー承認事項として残す。
