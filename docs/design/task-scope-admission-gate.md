...
18. 拒否上限の計上規則そのものを固定する。理由コードから計数先への写像、免除経路がクラス予算で有界であること（免除される理由コードごとに固定する）、逸脱試行が上限を枯渇させること、および **invocation から理由コードへの分類が両方向で正しいこと**（書込形・境界外読み取りは計上側、境界内の純粋な読み取りは免除側）。写像のみを固定すると、分類の誤りが検出されずに残る。
...
- **拒否上限（`max_denied_calls`）の計上は、write 境界・実行境界への逸脱試行に限定する**（D-S3-7 の決定、2026-08-23）。上限の目的は境界の反復的な探索を止めることであり、必要手順の一部である読み取り拒否や作業記録の拒否を計上すると、規定のフローに従うターンが自らの拒否で座礁する。上限値（6）は維持する。
  ...
  - **逸脱かどうかは invocation 全体（subcommand + 引数）の性質であり、subcommand の性質ではない。** subcommand 単位で免除を与えると両方向に誤分類が生じる（状態変更形が免除側へ落ち、境界内の純粋な読み取りが計上側へ落ちる）。許可された読み取り subcommand の拒否は**三分類**する。
    1. **ロック済み root の外を指す引数** → 計上（境界への逸脱試行）。`..` の判定はパス要素単位で行う。リビジョン範囲は `..` を演算子として運ぶため、文字列包含での判定は純粋な読み取りを逸脱と誤判定する。
  ...
  - **免除される subcommand 集合には、書込形を併せ持つ subcommand を入れない。** 読み取り形と状態変更形を同一 subcommand 名で兼ねるもの（リモート設定系、reflog 系）は認識外扱い（計上側）とする。免除側に置くと、write 境界への反復探索が拒否上限ではなく tool 予算でのみ有界になる。
