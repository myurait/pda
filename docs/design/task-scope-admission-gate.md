...
- 読み取り系ツール（read/search/list 系）: 許可（audit 記録のみ）。許可集合は実行中のツール語彙から導出し、実在しない名前を並べない（自動テストで語彙との一致を固定する）。
  - **エージェント自身の設定・メタデータ面の読み取り（スキル定義閲覧・ツール schema 閲覧）を許可集合に含める**（隔離実機検証の false deny 2 件の処置、2026-08-24。有効化承認シートの明示項目として批准対象）。分類根拠は「リポジトリのパスも実行境界も名指さないため、第一層が境界付ける宛先が存在しない」ことであり、この根拠はターンに依存しないため許可は全段（束縛不能経路を含む）に及ぶ。これが本追加の射程の全体である。スキル定義を**書き得る**ツールは従来どおりカタログ外（リポジトリのファイルを書き得るものの除外）であり、別ツールの呼び出しを引数に運ぶ形も同様に含めない。
...
**7. 隔離実機検証による確認（2026-08-24）**

第 3 項の束縛修正（17 節の処置、`bc32ca1`。以降の反証レビュー・再検証を経て確証済み）を、live 資産から分離した隔離環境（`HERMES_HOME` を専用ディレクトリへ切替）で実 LLM worker・実カードにより検証した。task_id 解決がカード識別子で結合し、seed 消費 → `locked` / `artifact-change` / `contract_origin=assignment` のターンとして write → stage → ローカルコミット → complete まで完走することを実測で確認した（scope 外の変異試行は正しく拒否）。全走行を通じ live 資産（本番 kanban.db・gate store・オーナーの作業ツリー）は不変である。検証記録は `docs/status/restricted-s3-impl-fix-2026-08-23-disposition.md` §20 を参照。

**実機で新たに判明した事項（実機確認済み・処置済み、批准は有効化ゲート）**: 読み取り専用ツール（スキル定義閲覧・ツール schema 閲覧）が「第一層: write 境界」節の読み取り系ツール許可集合に含まれないため拒否される事例を確認した。非計上・座礁なしだが純粋読み取りへの運用上の摩擦であり、D-S3-7 補則の litmus（読み取りを目的とする呼び出しが到達しうる理由コードは計上しない）と同じ思想に反する。処置として両ツールを当該許可集合へ加えた（上記「第一層: write 境界」節の該当項）。**本変更は許可集合を広げる方向であるため、反証レビューの対象とし、有効化承認シートの明示項目として批准を受ける。** 処置の記録は `docs/status/restricted-s3-impl-fix-2026-08-23-disposition.md` §21 を参照。
