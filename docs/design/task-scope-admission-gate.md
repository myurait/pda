...
- **D-S3-7（第一層の許可集合と運用手順の整合。オーナー判断事項、2026-08-23 起票）**: 第一層は現在、書込・ステージ・ローカルコミット・opt-in 検証のみを許可し、読み取り専用 git（`status` / `diff` / `rev-parse` / `branch` / `log`）と作業記録系ツール（kanban 系・todo 系）を拒否する。これは §11 第一層の列挙に対する厳密読みだが、(a) ステージ対象を列挙するための差分確認手段が契約内に無く、(b) 自律 worker レーンの必須手順（承認 metadata の base/head SHA・changed files・ブランチ同一性、カード更新）が契約内で完結せず、(c) それらの拒否が拒否上限を消費してターンを座礁させうる。判断事項は3つ: 読み取り専用 git を第一層へ加えるか、リポジトリ境界の外側にしか作用しないツール群を第一層の判定対象外カテゴリとするか、拒否上限を write 境界の逸脱試行に限定するか。**設計判断であるため実装側では変更していない**。§10 の受入項目一覧は closeout 事例を前提としており、artifact-change 版の受入項目（強制状態での replay fixture）の新設も同じ判断に含める。
- **D-S3-8（lock 前段の既定値の適用範囲。オーナー判断事項、2026-08-23 起票）**: 実装は lock 前段の既定拒否を全レーンで既定 off にしている。第8項の M2 分岐が免除しているのは「自律 worker レーンでの強制」だけで、seed 配線に依存しない自己 lock レーン（対話ターン）まで既定 off にするのは設計より広い。設定経路（環境変数）は M1 で入れたが、既定値は変更していない。
