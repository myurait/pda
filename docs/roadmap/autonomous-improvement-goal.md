...
- Status: draft-for-owner-approval（オーナー承認により効力を持つ。承認前は判断材料であり、ロードマップの改定ではない）
...
- アクティブ30枚（アーカイブ込み33枚）。実カードは約21枚: 完了5、インシデントtriage2（優先度2000/1990）、ready6（スコープゲート評価 t_65c9f889、即時計画表示の本番有効化 t_85f7cf5d、ポストモーテム t_efb0d5b2、自己認識注入 t_e4a13ad6 等）、オーナー起票triage8（並列worker化 t_c5638264、承認UX統合 t_05a29b7e / t_056cefd3、UI統一 t_125e89dc、ナレッジ化 t_ebb8de2f、delegate上限 t_bc4382f9 等）。
- 合成テストカード9件が盤面に可視のまま残る（"high" / "race" / "collision" / "not ready" 等。インシデント記載の混入は12件）。
- worker用worktreeの残骸が `~/projects/` 配下に残置され、加えてセッション作業由来のアドホックworktreeが11個存在する（pda-communication-integrity、pda-delegation-design 等）。サイクル外で行われた作業の物理的痕跡であり、GC機構はない。
...
- E2. Priority 0（通信誠実性）の扱いの確定。exit gateの主要素（5分cadenceの進捗契約・停滞表示・計画強制）は2026-08-22の修正とlive probeで実装・実証済みだが、closure宣言はオーナー判断（Open Question 1）。
...
- 内容: (a) t_4a78c98b — 試験の状態分離をfail-closed化し、混入12件と生成worktreeを証拠保全のうえ隔離、(b) t_40cb8c26 — 実施済み修正の証拠をカードへ消し込み、残条件（statusHistoryのmodel context非混入検証等）を閉じる、(c) t_877230c3 — terminal遷移のidentity束縛と再試行制御、(d) enabled状態の正本を1箇所に統合し、habit/policy/runtimeを導出値にする、(e) 盤面浄化（テストカードの隔離、残置worktreeのGC方針確立）、(f) リポジトリ外実体（kanban_db・不足スキル2件）の所在確認と契約の固定。
- exit gate: 盤面が実カードのみ・インシデント2件closed・回帰テストあり・状態正本が単一。何も再有効化されていないこと。
- オーナー関与: 混入カード隔離の承認、M0完了確認。
...
- 内容: (a) ループ全体を所有する単一オーケストレーターの導入（イベント駆動遷移、30分blind tickの廃止）、(b) WIP意味論の分離（実行中と承認待ちを別上限で管理し、承認待ちによる飢餓を解消）、(c) 並列worker枠（t_c5638264 要件1-8: 目標4枠、原子的claim、資源上限、毎時照合と回収）、(d) カード1:1のOpen WebUI専用スレッドと進捗・終端通知（同 要件9-15。表示契約は t_b18e8066 と共通化）、(e) 起票契約の統一（capture 5系統を単一契約へ）、(f) 日次reconcilerの再設計（C6準拠: イベント駆動化または成果消費が保証される形でのみ復帰）。
- exit gate: t_c5638264 の受入条件E2E（4枠並列・5件目待機・再起動復旧・終端通知）。盤面だけからセッション横断の作業状況が再構成できること。
- オーナー関与: 並列枠数と通知チャネルの確定、reconciler復帰形態の承認。
...
1. Priority 0（通信誠実性）をclosedとするか。証拠: 5分cadence契約の実装・本番反映・live probe実証（2026-08-22）。closedなら本再設計をactive priorityに位置付け、`docs/roadmap/current-priority.md` を改定する。
2. 本文書の承認と M0 着手可否。
3. 混入テストカード12件の隔離方法（アーカイブ＋証拠保全を提案）と、t_936048ad の承認キューからの除去。
4. 日次reconcilerの復帰形態（C6との両立。無条件の毎朝AI起動には戻さない前提で、イベント駆動化か、頻度・出力消費条件付きか）。
5. 全体設計発注書（`.hermes/prompts/claude-fable-full-system-design.md`）との関係。本再設計を全体設計の一部として進めるか、独立トラックとするか。
6. 多層認知ゲート（構想フェーズ8）との順序。本計画はM1で「自己改善に必要な最小ゲート群」を先行実装する方針だが、これはフェーズ8全体の代替ではない。この先行を許容するか。
