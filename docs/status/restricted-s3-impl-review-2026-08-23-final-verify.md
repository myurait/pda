...
## V4-0. 検証範囲と独立に確認できたこと

- 対象: `17b236b..6e61112`（V3-01 / V3-02 の処置と、司令塔決定「補則の定式化統一と帰属確定」の実装）。範囲は当該差分に限定した。
- 差分の構成: 設計 `task-scope-admission-gate.md`、実装 `scope_gate.py`、テスト `test_artifact_change_scope.py`、台帳 §13、および前ラウンドで未コミットだった本ファイルの V3 節。
- 検証道具は `tmp/verify-v4/` に置いた（`ast_diff.py` / `probe_strand.py` / `probe_oracle.py` / `fenceplug.py`）。git fixture は sandbox が repo 配下への入れ子 `.git` 作成を拒否するため `$TMPDIR`（pytest の `tmp_path` と同じ位置）に置いている。

独立に確認できたもの:

1. **計上集合の変更は計上 → 非計上の一方向のみ。** 両版を AST で解析して集合リテラルを抽出した。24 → 22、除去は `execution-not-opted-in` / `execution-template` の 2 件、**追加はゼロ**、変更前集合が変更後集合を真に包含する。
2. **admission は無変更。** ファイル全体の最上位ノード（132 個、両版で同数）を `ast.dump` で突き合わせ、**差分があるノードは集合リテラル 1 個のみ**であることを確認した。`_admit_artifact_change_execution` / `_admit_artifact_change_terminal` / `_admit_artifact_change_git` / `_admit_artifact_change_git_read` / `_admit_artifact_change_locked` / `_admit_artifact_change_pre_lock` / `_scan_template_tokens` / `_template_head_tail` / `artifact_deny_counter` はいずれも AST 一致。報告の「admission 関数を一切編集していない」は成立する。
3. **座礁の解消（実測）。** ロック済み worktree 内・write scope 内のファイルを対象とする読み取り目的の terminal 呼び出しを上限+2 回（8 回）反復した後、読み取りツール・`git status --porcelain`・write scope 内の書込・`git add` の 4 経路すべてが許可され、`denied_count == 0`（`tool_count == 12`）。opt-in の無い契約（`execution-not-opted-in`）と opt-in 済みで不一致の契約（`execution-template`）の両変種で同結果。設計 §10 受入項目18 の第 2 点に追記された固定対象と一致する。
4. **感度対照（プローブが失敗しうることの担保）。** 同一プロセス内で計上集合を旧 24 メンバーへ差し替えて再実行すると、両変種とも `denied_count == 6` で座礁し、以後は無条件許可の読み取りツールまで `deny-budget` を返す。V3-02 が報告した終状態を再現しており、修正が no-op でないことを示す。
5. **フェンス実効性（台帳 §13-5 の「7 失敗」の独立再現）。** 旧集合を注入して新規・改訂テストを実行し、**7 失敗**を確認した。内訳は同値性 1、非計上側列挙 2、座礁回帰 2、有界性 2 で、台帳の記述と一致する。
6. **拒否上限が無効化されていないこと。** write 境界の逸脱（write scope 外への書込）は 6 回で `deny-budget` に到達する。テンプレート起動形に一致した後の scope 外指定（`execution-target`）も 6 回で `deny-budget` に到達する。
7. **全数 506 passed**（`test_hermes_integration.py` を除外した規定コマンド）。

## V4-1. 確証欠陥

**無し（0 件）。** 焦点 4 項目のいずれについても、修正報告の主張と実測が一致した。

## V4-2. 焦点 4 項目の判定

### (1) 設計の帰属規則が単一定式化であり、個別帰属 1〜4 と一致する — 充足

- litmus が唯一の規範として単独提示されている（§11、blockquote）。レーン別の確定事実 5 点は「litmus の適用結果であり、litmus と並ぶ第二の規則ではない」「食い違った場合は列挙が誤り」と明記され、従属関係が文面で固定されている。
- 個別帰属の対応（司令塔決定 → 設計本文）:
  - 帰属 1（認識外 subcommand・認識外ツール = 非計上）→ §11 の「認識できなかったことを理由とする拒否は計上しない」および D-S3-7 追記の 1。
  - 帰属 2（workdir がロック済み worktree 外 = 非計上、字句解析後の再判定は M2 検討残余）→ §11 の「コマンドの字句解析より前に決まる拒否は計上しない」（M2 検討残余を明記）および追記の 2。
  - 帰属 3（ステージ・コミット引数形 = 計上維持、実行テンプレート不一致 = 非計上、起動形一致後の引数形 = 計上維持）→ §11 の「実行レーンの確定点はテンプレート起動形への一致である」および追記の 3。確定事実 #3 も「opt-in 済み実行テンプレートの起動形に一致すること」へ改まっている。
  - 帰属 4（冪等性ガード = 計上維持、litmus の明示例外）→ §11 の「litmus の明示例外（唯一）」（他コードへ例外を広げない旨も明記）および追記の 4。
- 旧極性の掃き出しを独立に確認した。設計全文の `計上` / `免除` の全出現を litmus に照らして読み、**規範として旧極性を述べる記述は残っていない**。補則の既定（非計上）と正面から矛盾していた一文（「未分類の拒否理由は計上する」）は削除済み。三分類節は「決めるのは audit 帰属ラベルと引数形 admission であり予算の帰属は決めない」の前置きの下で境界側／読み取り側ラベルへ retag されている。残る `免除` の出現は経緯記述・`test_paths` の非免除・M2 分岐の免除であり計上規則とは無関係。
- 指令文の litmus（「純粋な読み取り目的の呼び出しが到達しうる…」）と設計本文（「読み取りを目的とする呼び出しが到達しうる…」）は語形が異なるが、設計は直後に当該語を定義しており（読み取り系ツール、許可読み取り subcommand、レーン未確定の terminal コマンド）、22 メンバーおよび非計上側のいずれについても両読みで所属が一致する。計上先が反転するコードは存在しないため、記述上の差にとどまる。

### (2) 変更の一方向性と座礁の解消 — 充足

V4-0 の 1・3・4 のとおり。追加ゼロ・真の包含関係を AST で、座礁解消を実測で、対照の失敗を旧集合注入で確認した。

### (3) admission 無変更とオラクル面の維持 — 充足

- admission 無変更は V4-0 の 2 のとおり（全ファイルで変化した最上位ノードは集合リテラル 1 個のみ）。
- **非計上へ移した 2 コードが scope 内容のオラクルになっていないこと**を実測した。write scope を 3 通り（`src/*.py` / `other/*.py` / `secrets.txt`）に変えて同一コマンドを与えても、`execution-not-opted-in` / `execution-template` は action・reason ともに完全に不変であり、計上先も `tool_count` で不変。scope 内容に感応する情報は当該 2 コードを通らない。
- **scope 内容に感応する拒否コードは計上側に残っている。** 同じ 3 通りの scope に対し、テンプレート起動形に一致した後の `execution-target` は判定が変化し（scope 内なら許可、scope 外なら拒否）、拒否時の計上先は `denied_count`。scope パターン照合に依存する探索は引き続き拒否上限（6）で有界であり、tool 予算（96）へ緩んでいない。`write-scope` / `stage-scope` / `target-*` も集合に残存。
- 構造面でも、`execution-not-opted-in` / `execution-template` を返す 2 分岐は scope パターンにもファイルシステムにも触れない（`patterns` は `_scan_template_tokens` へ渡されるだけ）。
- 参考: `target-drift` の計上維持を確認した。artifact-change 側の当該コードは読み取り subcommand・認識外 subcommand・非 add/commit・権限欄の各分岐で早期 return した後にのみ到達するため、`git add` / `git commit` 専用であり litmus に適合する。closeout 側の同名コードは別クラスであり、計上規則はクラス条件で artifact-change に閉じている（呼び出し元 1 か所）。

### (4) 全数 green と台帳の数値一致 — 充足

- 506 passed。502 → 506 の内訳も一致（新規: 座礁回帰 2 ＋ 有界性 +2 = 4／改訂: 全数 24→22 の −2 と非計上側 18→20 の +2 で相殺ゼロ）。
- 実装の集合サイズ 22 は台帳 §13-3 の「24 → 22 メンバー」と一致。
- 収集数の実測と台帳 §13-5 の対応: 全数テスト 22、非計上側列挙 20、座礁回帰 2、有界性 4。いずれも一致。
- V3-01 の処置対象も一致を確認: §12-4-1 は「24 メンバー」＋誤記であった旨＋§13 での 22 への縮小への参照、§12-5 の全数テストは「24（13 節の縮小後は 22）」、見出しは「新規 43 パラメタ」。別義である §11-4 の「25 失敗」（フェンス実効性の失敗件数）は未変更。
- 「新規 43 パラメタ」の妥当性を履歴から検算した。`240d859` = 459 収集、`17b236b` = 502 収集、HEAD = 506 収集。§12 ラウンドの純増は 43 で、同節冒頭の「502 passed（本節前 459…）」と整合する。

## V4-3. 誤検知として棄却した候補（記録）

- **litmus の語形差（「純粋な読み取り目的」対「読み取りを目的とする」）** — 設計が当該語を直後に定義しており、両読みで 22 メンバーおよび非計上側の所属が一致する。計上先が反転するコードを構成できない。棄却。
- **§12-5 の「新規 43 パラメタ」が同節の新規テスト列挙の合計（59）と一致しない** — 43 は当該ラウンドの suite 純増（459 → 502）であり、同節冒頭の全数行と整合する。列挙の合計 59 との差は同節が併記する「既存 20 件の改訂」と「旧基準と矛盾する 2 件の置換」による減少分で説明される。収集数を実測して 459 / 502 / 506 を確認済み。数値としては正しく、V3-01 の処置（42 → 43）も正しい。棄却。
- **起動形一致後の `execution-option` が読み取り目的の呼び出しに到達しうるか**（例: 起動形に一致した上でヘルプ表示系の指定を与える形） — 設計は「一致した時点で呼び出しの目的を問わず実行境界への行為として確定する」と明示しており、litmus の「読み取り」はレーン未確定の呼び出しを指す定義語である。契約自身の opt-in が当該プログラムを名指している以上、起動形一致後を実行境界として扱う選択は内部整合的であり、必須手順は当該レーンに到達しない。棄却（設計の明示的選択）。
- **実行テンプレート不一致系の非計上により実行境界への反復が tool 予算のみで有界になること** — 司令塔決定が帰属 1 と同一性質の緩和として明示的に受け入れた帰結。admission は全形で拒否のままで、閉鎖は拒否（fail-open しない）ことを実測で確認済み。棄却（台帳 §13-7 の確認事項として既に起票されている）。

観察（欠陥ではない、将来ラウンドの任意の締め直し候補）:

- 計上集合のコメントは `target-*` 群に「Structured write lane: only a tool in the write catalogue reaches these」という排他形の見出しを与えているが、実測では `target-traversal` / `target-closed` は起動形一致後の実行レーン（`terminal`）からも到達する。**計上先の結論は変わらない**（起動形一致でレーンが確定しているため litmus に適合し、いずれの経路でも計上が正しい）。当該行は本差分で編集されておらず、挙動にも監査帰属にも影響しない。設計側の確定事実 #3 は当該経路を正しく被覆している。

## V4-4. 司令塔判断へ回す残余（本検証で増えたもの）

無し。台帳 §10-4 の 2 項目と §13-7 の 1 項目（不一致系レーンの有界性が拒否上限から tool 予算へ移ることの受容が、帰属 1 の批准と同一の判断であることの確認）をそのまま引き継ぐ。§12-7 の 4 項目は §13-1 の決定により閉じている。
