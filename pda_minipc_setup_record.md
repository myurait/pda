...
- stream全体をasync generatorの `finally` で包み、Hermes event中、最終content後、stop chunk後、`data: [DONE]` 後のどこでconsumerがcloseしてもntfy送信をscheduleし、通知タスクをPipe instanceが完了まで強参照する
- Pipe開始時のOpen WebUI host taskをDB読取前の順序barrierとしてterminal状態まで待つが、host task自体のsuccess・cancel・失敗は通知可否に使わない。streamがcancelで途中closeされた場合もDB確認を起動し、Open WebUIが保存した `done=true` の部分回答が空でなければ同じ形式で通知する
...
- Open WebUI内部呼び出しはtop-level/nested `internal` metadataに加え、v0.11.0のautomation・timer・subagent専用request pathでも除外する。`title_generation`、`tags_generation`、`follow_up_generation` taskも通知対象外
...
- Open WebUI Function APIにはrevision/CAS条件付き更新・削除がないため、installer失敗時の自動delete/rollbackは行わない。競合した管理画面/API更新を破壊しないことを優先し、原因確認後にidempotentなinstallerを再実行して整合させる。成功時はFunction source v2.1.0-local.11・active・全ValvesをAPIから再読込し、余分なValveも含め完全一致を確認する
...
- 単体・ローカル統合テスト: 57件すべて成功
- 実Open WebUI API → Progress Pipe → Hermes Runs API: 最終応答 `OWUI_PUSH_PREVIEW_OK` 後、チャットタイトル、回答冒頭、対象チャット直リンクを持ちemoji tagを持たないpushが1件だけ発生。期待通知の検出後も22秒間pollし、遅延した自動taskを含む余分な通知0件を確認
...
- 同じownerでもOpen WebUI frontend由来でない直接Runs API呼出しではpush 0件を確認
- 本番Functionはv2.1.0-local.11・active・Git管理sourceと完全一致。Open WebUI v0.11.0はhealthyで、反映後E2E期間の関連error logは0件
- Open WebUIのglobal default、管理者UI設定、E2E後の全34チャットが `hermes_progress_pipe` を使用することを再確認
- 外部平文HTTPや資格情報埋め込みURL、未許可user、Open WebUI内部taskではpushされないことを確認
