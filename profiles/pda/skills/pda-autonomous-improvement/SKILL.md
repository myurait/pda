...
5. Commit only the card's files on its task branch. Do not merge, push, deploy, restart services, alter runtime/profile state outside the worktree, send externally, change credentials, delete durable data, or perform an irreversible operation.
   ...
   - Never write to or stage anything under `.git`, and never create a repository inside the worktree. Git's own metadata is outside every scope contract however wide the card's write scope is, and both refusals count against the deny ceiling. This is decided by which entity the path opens, not by how the name is spelled, so another name for the same storage is refused the same way. Let Git manage its own directory.
