...
- Open WebUI内部呼び出しはtop-levelとnested user-message metadataの `internal` flagに加え、v0.11.0のautomation・timer・subagent専用request pathでも除外する。`title_generation`、`tags_generation`、`follow_up_generation` などのtask呼び出しも除外する。このため、内部taskや非同期エージェントの完了は同じpushを発生させない。

## 完了タイミングと表示内容

実装バージョンは `hermes_progress_pipe` v2.1.0-local.11。

ストリーミング時は、async generator全体を `finally` で包み、Hermes event中、最終content後、stop chunk後、`data: [DONE]` 後のどこでconsumerがcloseしてもntfy送信タスクを起動する。Pipe開始時のOpen WebUI host taskはDBを読む前の順序barrierとしてterminal状態まで待つが、host task自体のsuccess・cancel・失敗は通知可否に使わない。最終判断はOpen WebUI DB上の対象assistant messageが `done=true` か、所有者が一致するか、保存済み回答が空でないかだけで行う。これにより、保存されたfailure/cancel応答も通知しつつ、未保存または未完了の内容は外部へ送らない。通知タイトルと本文は保存済みレコードだけから読み、Hermesのterminal outputやユーザー入力を代用しない。
...
リポジトリ側のファイルを実稼働ディレクトリへ反映してからinstallerを実行する。installerは既存の同一ローカルFunctionだけを更新し、無関係なFunctionを上書きしない。Open WebUI Function APIにはrevision/CAS条件付き更新・削除がないため、失敗時の自動delete/rollbackは行わない。競合した管理画面/API更新を破壊しないことを優先し、失敗時は原因確認後にidempotentなinstallerを再実行して整合させる。成功時はsource、active state、全ValvesをAPIから再読込し、余分なValveを含めて完全一致を確認する。
...
このlive probeは、通知タップ先も検証できるよう `通知・進捗E2E確認 ...` というテストチャットを1件残す。assistant messageの `done=true`、`Hermesが処理を開始しました…` と `完了` のstatus履歴、pushが厳密に1件であることを確認する。期待push検出後も22秒監視し、遅延した自動タイトルtask等による余分な通知がないことを否定する。失敗時は作成したチャットを削除する。
