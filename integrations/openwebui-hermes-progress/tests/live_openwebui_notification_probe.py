...
POST_NOTIFICATION_QUIET_SECONDS = 22
...
async def collect_notification_window(
    *,
    fetch_events: Callable[[], Awaitable[list[dict]]],
    baseline_ids: set[str],
    started_at: int,
    is_expected: Callable[[dict], bool],
    ...
    wait_deadline = monotonic() + initial_wait_seconds
    ...
    while True:
        ...
        if expected_seen_at is None and any(
            is_expected(event) for event in new_messages.values()
        ):
            expected_seen_at = now

        if expected_seen_at is not None:
            if now - expected_seen_at >= quiet_seconds:
    ...
    async with aiohttp.ClientSession(timeout=timeout) as client:
        try:
            ...
            async def fetch_events() -> list[dict]:
                return await ntfy_messages(client, ntfy_server, ntfy_topic)

            new_messages, quiet_window_complete = await collect_notification_window(
                fetch_events=fetch_events,
                baseline_ids=baseline_ids,
                started_at=started_at,
                is_expected=lambda event: (
                    event.get("message") == EXPECTED_ANSWER
                    and event.get("click") == expected_click
                ),
                initial_wait_seconds=50,
                quiet_seconds=POST_NOTIFICATION_QUIET_SECONDS,
                poll_seconds=0.25,
                monotonic=loop.time,
                sleep=asyncio.sleep,
            )
            ...
            result = {
                ...
                "completion_push_quiet_window_complete": quiet_window_complete,
            ...
            assert quiet_window_complete is True, result
